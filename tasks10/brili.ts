// From https://github.com/sampsyo/bril
...
let seenCount = new Map<string, Map<string, number>>();
let isTracing = false;
const activeTrace: string[] = [];
const traced: Set<String> = new Set<string>();
const traces: string[][] = [];
  ...
  for (let i = 0; i < func.instrs.length; ++i) {
    ...
    if ("op" in line) {
      ...
      if ("label" in action) {
        ...
        if (tracingEnabled) {
            let funcFreq = seenCount.get(func.name) || new Map<string, number>();
            let labelFreq = funcFreq.get(action.label) || 0;
            funcFreq.set(action.label, labelFreq + 1);
            seenCount.set(func.name, funcFreq);
            if (isTracing == true && action.label == activeTrace[0]) {
                traces.push(activeTrace.slice())
                while (activeTrace.length != 0) {
                    traced.add(activeTrace.pop());
                }
                isTracing = false;
            }
            if (isTracing == true) {
                activeTrace.push(action.label)
            }
            if (labelFreq >= 5 && isTracing == false && !traced.has(action.label)) {
                isTracing = true;
                activeTrace.push(action.label);
            }
        }
